...
const buildSeats = (rows) => {
    return rows.flatMap((row) => Array.from({ length: row.seatCount }).map((_, idx) => {
...
async function main() {
    const passwordHash = await bcryptjs_1.default.hash(env_1.env.ADMIN_PASSWORD, 10);
    const admin = await prisma.user.upsert({
        where: { email: env_1.env.ADMIN_EMAIL },
        update: {
            fullName: env_1.env.ADMIN_NAME,
            passwordHash,
            role: "ADMIN",
        },
        create: {
    ...
    const genres = await Promise.all(genreNames.map((name) => prisma.genre.upsert({
        where: { name },
        update: {},
        create: { name },
    })));
